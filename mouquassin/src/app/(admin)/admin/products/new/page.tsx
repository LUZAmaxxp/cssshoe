...
import { CloudUpload, X, ChevronRight, ChevronLeft, Check, Package, Plus, Palette } from "lucide-react";
    ...
    try {
      ...
      const res = await fetch("/api/products", {
        ...
        body: JSON.stringify({
          ...
          price: Number(price),
          category,
          images: allImages.length > 0 ? allImages : colors[0]?.images || [],
          sizes: allSizes,
          colors: colors.map((c) => ({
            name: c.name,
            hex: c.hex,
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <p className="text-sm font-medium text-charcoal">
                  {colors.map((c) => `${c.name} (${c.images.length} imgs, ${c.sizes.length} sizes)`).join(", ") || "None"}
                </p>
              </div>
            </div>
