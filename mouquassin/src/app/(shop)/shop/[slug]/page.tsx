...
export default function ProductDetailPage() {
  ...
  return (
    <>
      ...
      {/* Size Guide Modal */}
      {showSizeGuide && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={() => setShowSizeGuide(false)}>
          <div className="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <h3 className="text-sm tracking-wider uppercase text-charcoal">{t("product.sizeGuide")}</h3>
              <button onClick={() => setShowSizeGuide(false)} className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-charcoal transition-colors">&times;</button>
            </div>
            <div className="p-4">
              <img
                src="/images/guide_size.PNG"
                alt="Size Guide"
